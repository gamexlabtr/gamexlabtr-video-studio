...
const GAME_CATEGORY = process.env.GAME_CATEGORY || 'HTML5 Games';
const rawSeconds = Number(process.env.RECORD_SECONDS || 15);
const RECORD_SECONDS = Math.max(8, Math.min(30, Number.isFinite(rawSeconds) ? rawSeconds : 15));
...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(outputDir, 'cover.png'), fullPage: false });
